...
train=fread("~/Desktop/Data/Kaggle/RedHat/act_train.csv", verbose = FALSE) %>% as.data.frame() #added verbose for silent
test=fread("~/Desktop/Data/Kaggle/RedHat/act_test.csv", verbose = FALSE) %>% as.data.frame() #added verbose for silent
...
write.csv(sub, file = "~/Desktop/Data/Kaggle/RedHat/model_sub.csv", row.names = F)
...
ppl <- fread("~/Desktop/Data/Kaggle/RedHat/people.csv")
...
cat("Working on data\n")
# read and combine
activs <- fread("~/Desktop/Data/Kaggle/RedHat/act_train.csv")
TestActivs <- fread("~/Desktop/Data/Kaggle/RedHat/act_test.csv")
...
d1[, activdate := as.Date(as.character(date.y), format = "%Y-%m-%d")]
...
interpolateFun <- function(x){
  ...
  #vec <- sqrt(forward_x_fill * backward_x_fill) #geometric mean
  vec <- (forward_x_fill + backward_x_fill)/2 #average
...
write.csv(testsetdt,"~/Desktop/Data/Kaggle/RedHat/Submission.csv", row.names = FALSE)
...
submit1 <- fread("~/Desktop/Data/Kaggle/RedHat/Submission.csv")
submit2 <- fread("~/Desktop/Data/Kaggle/RedHat/model_sub.csv")
...
write.csv(submit5, file="~/Desktop/Data/Kaggle/RedHat/YOUNG_4538.csv", row.names=FALSE)
